...
def main():
    ...
    try:
        ...
        if any(_ in excMsg for _ in ("MemoryError", "Cannot allocate memory")):
            ...
            errMsg = "permission error occurred while accessing file '%s'" % (match.group(1) if match else "unknown")
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = "wrong initialization of 'pymysql' detected (using Python3 dependencies)"
            logger.critical(errMsg)
            raise SystemExit
