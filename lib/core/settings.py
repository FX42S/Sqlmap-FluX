...
# =============================================================================
# WAF/IPS 规避 - 动态定界符生成 (Phase 1: Static Fingerprint Randomization)
# =============================================================================

def generateDynamicDelimiter(length=16):
    """
    生成动态随机定界符，模拟业务数据格式以避免 WAF 指纹检测
    支持多种格式：JSON片段、Base64风格、UUID风格、随机十六进制
    """
    import base64
    
    formats = [
        # JSON 响应片段风格
        lambda: '{"id":"%s","data":"' % ''.join(random.choice(string.hexdigits) for _ in range(length)),
        lambda: '{"ref":"%s","obj":{' % ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(length)),
        lambda: '{"_":"%s","v":[' % ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(length)),
        # Base64 填充风格
        lambda: base64.b64encode(os.urandom(length // 2 * 3)).decode()[:length],
        # UUID 风格
        lambda: '%s-%s-%s-' % (
            ''.join(random.choice(string.hexdigits) for _ in range(8)),
            ''.join(random.choice(string.hexdigits) for _ in range(4)),
            ''.join(random.choice(string.hexdigits) for _ in range(4))
        ),
        # 随机十六进制字符串
        lambda: ''.join(random.choice(string.hexdigits) for _ in range(length)),
        # 模拟追踪ID风格
        lambda: 'trace_%s_' % ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(length - 6)),
        # 模拟时间戳+随机数风格
        lambda: '%d%s' % (int(time.time()), ''.join(random.choice(string.digits) for _ in range(6))),
    ]
    
    chosen = random.choice(formats)()
    return chosen[:length] if len(chosen) > length else chosen

def generateRandomizedBoundaries():
    """
    生成随机化的定界符前缀和后缀，替代固定的 q...q 格式
    """
    # 生成随机的开始和结束标记
    start_marker = generateDynamicDelimiter(random.randint(8, 16))
    end_marker = generateDynamicDelimiter(random.randint(8, 16))
    return start_marker, end_marker

# 动态生成定界符（在启动时随机化）
KB_CHARS_BOUNDARY_CHAR = generateDynamicDelimiter(8)
KB_CHARS_LOW_FREQUENCY_ALPHABET = "zqxjkvbp" + generateDynamicDelimiter(4)

# 动态 Payload 定界符 (替代 __PAYLOAD_DELIMITER__)
DYNAMIC_PAYLOAD_DELIMITER_START, DYNAMIC_PAYLOAD_DELIMITER_END = generateRandomizedBoundaries()

# 动态 Base64 边界标记
BOUNDED_BASE64_MARKER_PREFIX = generateDynamicDelimiter(6)
BOUNDED_BASE64_MARKER_SUFFIX = generateDynamicDelimiter(6)
