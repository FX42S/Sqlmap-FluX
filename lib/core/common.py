...
# 模拟人类点击行为：有时连续发两个包（模拟快速点击），有时停顿 3-5 秒（模拟加载）
# =============================================================================

def poissonRandomDelay(baseDelay=None, lambda_param=2.0):
    """
    基于泊松分布的随机延迟生成器
    
    Args:
        baseDelay: 基础延迟时间（秒），如果为 None 则使用 conf.delay
        lambda_param: 泊松分布参数，控制分布形状
    ...
    if baseDelay is None:
        baseDelay = conf.delay if hasattr(conf, 'delay') and conf.delay else 0
    
    # 如果基础延迟为 0，返回 0
    if baseDelay <= 0:
        return 0
    
    # 泊松分布随机变量生成（使用 Knuth 算法）
    ...
    # 模拟人类行为模式：
    # - 快速点击模式（20% 概率）：延迟时间缩短 50-70%
    # - 正常加载模式（60% 概率）：延迟时间在基础值附近波动
    # - 慢速加载模式（20% 概率）：延迟时间增加 50-150%
    
    behavior_roll = random.random()
    
    if behavior_roll < 0.2:
        # 快速点击模式
        multiplier = random.uniform(0.3, 0.5)
    elif behavior_roll < 0.8:
        # 正常加载模式
        multiplier = random.uniform(0.8, 1.2) * (1 + poisson_value * 0.1)
    else:
        # 慢速加载模式
    ...
    # 添加一些随机噪声（高斯分布）
    noise = random.gauss(0, baseDelay * 0.1)
    delay = max(0, delay + noise)
    
    return delay

def humanLikeDelay(baseDelay=None):
    """
    拟人化的延迟函数
    在基础延迟的基础上添加随机性，模拟人类操作模式
    """
    import time
    
    delay = poissonRandomDelay(baseDelay)
