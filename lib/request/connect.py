...
class Connect(object):
    ...
    def getPage(**kwargs):
        ...
        # =============================================================================
        # Phase 3: 非线性时间分布 - 泊松分布随机延迟
        # =============================================================================
        if isinstance(conf.delay, (int, float)) and conf.delay > 0:
            # 使用拟人化延迟替代固定延迟
            from lib.core.common import humanLikeDelay
            humanLikeDelay(conf.delay)
        ...
        try:
            ...
            # =============================================================================
            # Phase 1: 垃圾参数干扰 - 添加随机无关参数以对抗 URL 频率限制
            # 在每个请求中随机添加无关紧要的参数，使每个 URL 的哈希值都不同
            # =============================================================================
            if not any((refreshing, crawling, checking, direct_)) and target and not conf.liveCookies:
                # 生成随机的垃圾参数池
                junk_params_pool = [
                    ("_ts", str(int(time.time()))),
                    ("_v", "%d.%d" % (random.randint(1, 99), random.randint(0, 99))),
                    ("_nonce", ''.join(random.choice(string.hexdigits) for _ in range(8))),
                    ("_ref", ''.join(random.choice(string.ascii_lowercase) for _ in range(6))),
                    ("_cache", str(random.randint(100000, 999999))),
                    ("_rid", ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(12))),
                    ("_t", str(random.randint(1000000000, 9999999999))),
                    ("_track", ''.join(random.choice(string.hexdigits) for _ in range(16))),
                ]
                # 随机选择 1-2 个垃圾参数
                selected_junk = random.sample(junk_params_pool, random.randint(1, 2))
                junk_str = DEFAULT_GET_POST_DELIMITER + DEFAULT_GET_POST_DELIMITER.join("%s=%s" % (k, v) for k, v in selected_junk)
