...
def checkSqlInjection(place, parameter, value):
    ...
    while tests:
        ...
        try:
            ...
            # are below the corresponding configuration's level and risk
            # values
            ...
            _dbms = payloadDbms[0] if isinstance(payloadDbms, list) and payloadDbms else payloadDbms
            Backend.forceDbms(_dbms)
...
def heuristicCheckSqlInjection(place, parameter):
    ...
    # =============================================================================
    # Phase 3: 探测逻辑乱序化 - 先发正常请求做"垫底"，对抗态势感知
    # =============================================================================
    # 发送 1-3 个正常请求作为"垫底"，模拟正常用户行为
    num_baseline_requests = random.randint(1, 3)
    for _ in range(num_baseline_requests):
        try:
            # 发送原始值请求
            agent.payload(place, parameter, newValue=origValue)
            Request.queryPage(agent.payload(place, parameter, newValue=origValue), place, content=False, raise404=False)
        except:
            pass
        
        # 随机延迟，模拟人类操作间隔
        if isinstance(conf.delay, (int, float)) and conf.delay > 0:
            time.sleep(random.uniform(0.5, 1.5) * conf.delay)
    ...
    # Phase 3: 随机插入针对不存在参数的探测（噪声注入）
    if random.random() < 0.3:  # 30% 概率插入噪声
        try:
            fake_param = "_" + randomStr(length=random.randint(4, 8), alphabet=string.ascii_lowercase)
            fake_value = randomStr(length=random.randint(5, 10))
            # 构造带有不存在参数的请求
            noise_payload = "%s%s=%s" % (DEFAULT_GET_POST_DELIMITER, fake_param, fake_value)
            # 这里只是模拟，实际通过其他方式发送
            logger.debug("Inserting noise parameter for anti-detection: %s" % fake_param)
        except:
            pass
